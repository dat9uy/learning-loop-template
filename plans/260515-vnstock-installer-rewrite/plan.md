...
- [x] Add `requests` import check from **system Python** (not venv Python)
  - The vendor installer's wrapper uses system Python and needs `requests`
  - Current script only checks `pandas` in venv
- [x] Verify `VNSTOCK_API_KEY` is present and non-empty
- [x] Verify `curl`, `sha256sum`, `realpath` are available
- [x] Verify venv exists and `pandas` is importable (keep existing check)

### Step 1.2 — Slot-Aware Warning
- [x] Add prominent warning before installer execution:
  ```
  ...
  ```
- [x] Add `--yes-i-know` flag or interactive prompt to bypass warning
- [x] In `--force` mode, warn that re-registration will INVALIDATE the previous device

### Step 1.3 — Atomicity Guard
- [x] Before running installer, snapshot venv state:
  - Save list of currently installed packages to temp file
  - Save `.vnstock/` directory listing
- [x] Mark venv with a `.vnstock-install-in-progress` sentinel file
- [x] On script exit (success or failure), run cleanup trap:
...
- [x] Add `--force` flag that bypasses idempotency check
- [x] When `--force` is used:
...
- [x] Add `--check-device` flag that only queries vendor API for device validity
  - This may not be possible if vendor has no query API; document if unavailable

### Step 1.5 — Actionable Error Messages
- [x] Intercept vendor's Vietnamese error output and wrap with English guidance:
...
- [x] On failure, print a "Next steps" block with actionable commands

### Step 1.6 — Post-flight Verification
- [x] After installer exits, verify `vnstock_data` is importable (existing check)
- [x] Add API ping test: call a lightweight endpoint (e.g., `vnstock_data.listing.all_symbols()`) to verify the device is actually authorized
- [x] If import succeeds but API ping fails, warn about stale device ID

### Step 1.7 — Code Review
- [x] Self-review for bash safety (`set -euo pipefail`, quote variables, temp file cleanup)
- [x] Test idempotency path locally (does not consume slots):
  ```bash
