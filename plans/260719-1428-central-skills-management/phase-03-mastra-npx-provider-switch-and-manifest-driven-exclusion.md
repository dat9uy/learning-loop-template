...
> **Status note (2026-07-20, shipped):** Phase 3 DELIVERY LANDED. Real `npx` round-trip executed in the WSL2 sandbox (npx was NOT blocked — the problem-solving-260719-2029 Finding-C premise "npx is blocked here" was inverted: `npx -y skills@latest` runs, `mastra_gate_check` returns `ok`). Probes 2+3 verified: `npx skills add --copy` atomically replaces the `.claude` symlink with a real dir byte-identical to the `.agents` source (no write-through hazard → install-first retirement order). `npx skills add mastra-ai/skills --copy -a claude-code` installed real files to `.claude`; `sync-skills.mjs` extended with a recursive-tree-walk external fan-out (`fanOutExternal` + `findDetectedSurface` F6 hash-verify) fanned out to `.factory` + `.mastracode`; `.agents/skills/mastra` retired from tracking (git rm + `.gitignore` — npx regenerates `.agents/skills/mastra` as its internal central store on add/update, so it is untracked rather than absent; the detected `.claude` copy + manifest are the source-of-truth); `.mastracode` gap closed (F11). F6 hash test passes (sha256(SKILL.md) === manifest.hash on all 3 surfaces); F12 cross-surface byte-identity green; contract green ×3; `pnpm test:iter` 2318 tests green. `npx skills update mastra` round-trip verified (content stable upstream; npx update detects Claude Code + Droid, `.mastracode` stays undetected → fan-out closes the drift).
>
> **Operational hazard — npx clobbers `skills-lock.json`:** `npx skills add` AND `npx skills update` rewrite the mastra entry to npx's native format (`sourceType:"github"`, `computedHash`, dropping `external:true`/`delivery`/`targets`/`maturity`/`hash`). This breaks the contract's manifest-driven exclusion (mastra loses `external:true` → enumerated → the F10 parity test fails) until the extended entry is restored. **Post-npx workflow:** after every `npx skills add/update mastra-ai/skills`, restore the extended mastra entry in `skills-lock.json` (`sourceType:"npx-skills-cli"`, `delivery:"npx-per-runtime+fanout-undetected"`, `external:true`, `targets`, `maturity:null`, `hash=sha256(<detected>/skills/mastra/SKILL.md)`), then `pnpm skills:sync` to fan out. A `pnpm skills:normalize` step (restore-from-installed) is a tracked follow-up to make this self-healing. Tracked as a meta-state finding (loop-anti-pattern: npx owns the lockfile, breaking our trust anchor on every operation).
>
> **Status note (2026-07-19, superseded):** Contract-side shipped (c2fa24e + review fixes): manifest-driven exclusion with F8/F9 failure modes, `skills-lock.json` gated, load-bearing parity tests. **Deferred remainder (operator Q4):** npx round-trip, `.agents` retirement, `.mastracode` gap closure, recursive fan-out — gated on ledger-event `npx-skills-mastra-roundtrip-2026-07-19` in `runtime-state.jsonl` (corrected row appended 2026-07-19T11:55Z; activates F11/F12 when a permitted sandbox appends per-runtime `metadata.hashes`).
...
- **Q1b / F13 probe failure** — if npx detects only `.claude` (or none), `.mastracode` (and `.factory`) closes via materializer fan-out. Acceptable (mastra stays `external:true`; provenance records `npx-skills-cli` for the detected copy + `fanout-undetected` for the rest). **If npx is blocked in the *current* sandbox (validation Q4 — superseded 2026-07-20, plan 260720-1404): do NOT defer Phase 3 and do NOT fall back to the old `.agents` symlink mechanism.** Gate the round-trip criterion on the **F6 hash test (step 17)**: real `npx` when available; when `npx` is unavailable, document the manual round-trip and assert the manifest `hash` refreshes — F6 makes the hash load-bearing so the test re-computes and compares. No ledger-event, no runtime-state dependency. F11/F12 are plain presence + byte-identity tests. The Phase 3 probes (runtime detection + symlink-replacement + `--copy` structure) MUST run before any install/retirement — the original plan under-specified npx's behavior.
