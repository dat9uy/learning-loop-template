...
> **Status note (2026-07-20):** Contract-side shipped (c2fa24e + review fixes): manifest-driven exclusion with F8/F9 failure modes, `skills-lock.json` gated, load-bearing parity tests. **Deferred remainder:** npx round-trip, `.agents` retirement, `.mastracode` gap closure, recursive fan-out — gated on the F6 hash test (step 17): real `npx` when available, hash-recompute-and-compare when not. F11/F12 are plain presence + byte-identity tests with no runtime-state dependency. (Q4 ledger-event hand-off dropped 2026-07-20 — see plan 260720-1404.)
>
