...
- [ ] `core/placement.yaml` exists; enumerates every production `.js`/`.cjs`/`.mjs` file under `core/` (excluding `__tests__/`, `lib/`, `node_modules/`, and `*.test.js` at any depth — verified baseline = 27 files)
