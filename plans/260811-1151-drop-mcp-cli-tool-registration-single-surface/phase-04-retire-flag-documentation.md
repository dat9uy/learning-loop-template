...
actual 42 CLI / 44 manifest). Historical journals stay intact.
...
3. `CLAUDE.md`: rewrite the "Tool surface" bullet — remove the
   ...
   sentence; state the CLI carries the full surface (42 tools = 12 reads + 30 writes) and MCP
   keeps the irreducible residue. Correct "41 tools" → 42 (verify via
   `node -e "import('./tools/learning-loop-mastra/core/cli-tools.js').then(m=>console.log(m.CLI_TOOLS.size))"`).
   Keep `LOOP_SURFACE` / `GATE_ROOT` guidance. [Findings 7, 11]
...
6. Whole-repo verification:
   `grep -rn "LOOP_RECORDS_VIA_CLI\|LOOP_READS_VIA_CLI" . --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=plans`
   — expect zero evergreen hits. Historical hits under `docs/journals/` are acceptable; if
   any evergreen doc remains, rewrite it.
7. Verify all cited file paths still resolve; confirm `core/cli-tools.js` link target.
...
- [ ] `CLAUDE.md` "Tool surface" bullet matches the single-surface contract; count corrected to 42.
