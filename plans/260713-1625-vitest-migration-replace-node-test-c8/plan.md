...
description: "Migrate the node:test suite to vitest + @vitest/coverage-istanbul. Vitest's native --reporter=json gives the agent a trustworthy structured-failure document (numFailedTests + assertionResults) for free — dissolving the agent-context bleed that motivated the abandoned TAP-wrapper plan (260713-1503). Coverage uses vitest's istanbul provider (Istanbul-format coverage-final.json, the shape fallow:gate requires); the v8-provider path was abandoned in the shadow phase. The plan's claim that fallow's vitest plugin registers *.test.* as entry points (retiring the .fallowrc test-ignore workaround) was FALSIFIED in Phase 1 — fallow 3.3.0 has no such plugin; the 4 ignore lines were restored. Red-team corrected 3 critical defects (per-commit hook atomicity, 6 before(fn,{timeout}) call sites, 12 security-gate tests outside the include glob) and 2 high defects (gate should be fallow:gate not %-parity; 3 auxiliary node:test scripts)."
status: completed
priority: P2
branch: "main"
...
3. **Retires the `.fallowrc.json` workaround.** *(Plan assumption — **falsified in Phase 1**.)* Under node:test, fallow has **no plugin** → test files aren't entries → 4 ignore lines (`**/*.test.{js,cjs}`, `**/*.spec.{js,cjs}`) are required (else 212 false-positive `unused-file`). The plan claimed fallow's **vitest plugin registers `*.test.*` as entry points** (empirically confirmed via scratch probe). **This was wrong for fallow 3.3.0** (the pinned version): `fallow list --entry-points` returns only manual-entry files, no vitest plugin. The 4 ignore lines were restored in `9b26d12`; `r2/fallow-test-tree-clean.test.js` now asserts they REMAIN. Retiring them is deferred until fallow ships such a plugin (closeout follow-up #4).
...
- [x] `pnpm test` runs the full suite under **vitest**, exits 0 on full-pass / 1 on any failure, emits a **test-results JSON** (`numFailedTests`, `assertionResults[]`) at `.test-logs/vitest-results.json`.
- [x] **208** test files run under vitest (1876 tests, 1 skipped) — `tools/` + `.claude/coordination` + `.factory/hooks` + the codemod unit tests. (Plan's original "234 / 222+12" figure was a pre-prune estimate; Phase 0 pruned 21 dead/redundant files. Only 3 of the 12 gate tests were script-style and needed hand-wrap; the other 9 already used `describe`/`it`/`test`.)
- [x] The 5 `before|after|beforeEach|afterEach(fn, {timeout:N})` call sites rewritten to `(fn, N)` (the 6th, `debug/agent-e2e-integration`, was pruned in Phase 0); `test.hookTimeout:120000` set.
- [x] `coverage/coverage-final.json` produced by vitest's **istanbul provider** (`@vitest/coverage-istanbul`) in Istanbul format; `sanitize-coverage.mjs` runs on it; **`fallow:gate` exits 0** (the blocking gate). _(Deviation from plan: the config uses the istanbul provider, not the v8-provider + AST-remap. The v8-provider path was abandoned after the shadow phase — `@vitest/coverage-v8` was dropped from devDeps in the post-review cleanup; only `@vitest/coverage-istanbul` remains.)_
- [x] `tools/scripts/run-pnpm-test-namespaced.mjs` deleted; `c8` dropped from devDeps; `vitest@4.1.10` + `@vitest/coverage-istanbul` added.
- [ ] ~~The 4 `.fallowrc.json` ignore lines deleted; `r2/fallow-test-tree-clean.test.js` asserts fallow reports 0 `unused-file` AND `list --entry-points` shows `*.test.*` with `source:"vitest"`.~~ **NOT MET — reverted in 9b26d12.** Fallow 3.3.0 (the pinned version) has **no vitest plugin**; the plan's "empirically confirmed" plugin claim was false. The 4 ignore lines were restored; `r2/fallow-test-tree-clean.test.js` now asserts the lines **REMAIN** (with a comment pointing at a future fallow upgrade). Tracked as follow-up #4 in the closeout journal.
