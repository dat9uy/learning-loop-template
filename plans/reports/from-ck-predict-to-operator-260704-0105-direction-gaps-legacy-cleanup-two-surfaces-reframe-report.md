...
**Status (updated 2026-07-06; scope broadened 2026-07-07):** Rec 12 is **design-agreed** (implementation pending). The scoped rule shipped in plan 260704-0301 Phase 3 (close flow: fix for a dispatched finding IS a change-log trigger; operator/agent symmetric). The **general** trigger rule + Q11 (operator/agent symmetry) are design-agreed 2026-07-06 in `plans/reports/brainstorm-260706-0958-record-lifecycle-authority-redesign-report.md`, implementation pending in plan `260706-0958-record-lifecycle-authority-redesign`. Trigger concept: *an action becomes a change-log when it changes a bound artifact (concept- or implementation-surface doc, runtime contract, registry schema, tool manifest, tracker lifecycle, or `tools/**`/`core/**` source) or a rule/policy; not for in-session scratch, plan drafts, or reversible edits inside a not-yet-shipped plan.* Symmetry (Q11): **no operator exemption** — operator edits are recorded symmetrically with agent edits (escape-hatch #13), so `meta_state_log_change` is *trigger-gated* by Rec 12, not authority-gated, and fires for both.
...
12. **State the change-log trigger as a concept-surface rule.** **[DESIGN-AGREED 2026-07-06; scope broadened 2026-07-07 to three components]** In `loop-engine.md` (L1) under the `record` role, or as a short consult-gate: *an action becomes a change-log when it changes a bound artifact (concept- or implementation-surface doc, the runtime contract, registry schema, tool manifest, tracker lifecycle, or `tools/**`/`core/**` source) or a rule/policy. It does not become a change-log for in-session scratch, plan drafts, or reversible edits inside a not-yet-shipped plan.* This makes the `record` role's trigger queryable and ends the operator/agent ambiguity. Pairs with Rec 9 (procedure → loop-encoded, not doc'd): the trigger rule is the kind of procedure that belongs in a consult-gate, not prose. **Symmetry (Q11) agreed:** no operator exemption — `meta_state_log_change` is trigger-gated, not authority-gated, and fires symmetrically for operator and agent edits (escape-hatch #13). Implementation pending in plan `260706-0958-record-lifecycle-authority-redesign`.
   ...
   **2026-07-07 broadening — Rec 12 is now three components (the change-log closed loop), not one:**
   ...
   - **(b) Change-log gap detection** — detect bound edits that landed without a change-log; the change-log recurrence query that is the unwired half named in `loop-engine.md` open Q1. Read-time. Mechanism: join of bound-artifact paths touched (git/file-index) minus `meta_state_log_change` entries in the same session → gap finding. **[DESIGN-AGREED 2026-07-07, implementation pending]**
   - **(c) Session-start gap injection** — surface the change-log gap at session start so a fresh session sees it (state-2 deterministic injection, paralleling the existing stale-queue session-start surfacing). This is what makes the gap measurable across sessions and lets the loop *know* when manual invocation drifts. **[DESIGN-AGREED 2026-07-07, implementation pending]**

   The three compose into the change-log closed loop: **trigger (a) → detect gap (b) → surface gap (c) → promote recurring gap to deterministic hook.** The SessionEnd/pre-commit hook (skill-layer prerequisite UQ5) is the promotion step — downstream of this Rec 12, not part of it. (b)+(c) are the un-block condition for UQ5. **Plan home (decided 2026-07-07):** (a) ships in the lifecycle plan `260706-0958`; (b)+(c) ship in a dedicated follow-on Rec 12-closed-loop plan (not 0958's trimmed P4, not the skill-layer prerequisite); the hook ships downstream in the skill-layer prerequisite.
