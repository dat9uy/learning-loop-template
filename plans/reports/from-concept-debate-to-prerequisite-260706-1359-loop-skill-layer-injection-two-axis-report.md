...
**Status:** design-agreed, **not implemented**. No plan dir exists for this prerequisite or for the lifecycle-redesign plan (`plans/260706-0958-...` not cut). The L1 framing this rests on ships via `plans/260706-1340-philosophy-agents-two-axis-injection-reframe/` (the docs-rewrite plan, cut). Recent commits (#33–#35) shipped dispatch/close-flow; the skill layer and lifecycle redesign are untouched in code. **2026-07-07 consensus session** (see § "Consensus direction" below) resolved UQ1/UQ2/UQ3 and locked the direction: ship the chosen scope (A) as the layer with the consult-gate as the first enforcement surface; mirror via write-fanout (e); `skill_manage` dropped. Ready to cut the prerequisite plan.
...
## Consensus direction (2026-07-07 session)

A `/ck-predict` 5-persona debate on "which direction for the skill layer" + a `/ck:agentize` consultation on mirror dedup produced a locked direction. **Verdict: CAUTION → GO with A (the chosen scope above) as the layer, carrying the consult-gate (B) as the first enforcement surface — not B alone.** C (build the full skill-management framework now) rejected as YAGNI — verified `skill_manage` has zero consumers, no non-operator creates skills, and the Rec 12 trigger skill ships in a later plan. D (do nothing) rejected because the L1 framing alone doesn't satisfy Rec 9 (wiring does; without the layer the Rec 12 trigger skill is a second ad-hoc skill *beside* the loop — consumed state-1, not maintained state-2).

**Three UQs resolved:**

- **UQ1 (skill vs consult-gate) — RESOLVED by the UQ2 reframe.** The skill realizes the state-2 injection of a loop-design's context (deterministic injection + agentic consumption); the consult-gate is the state-3 guardrail on the loop-design's governed actions (deterministic consumption of the guardrail). They **compose, not compete**: the skill carries the content; the consult-gate enforces the boundary. The consult-gate is the first *enforcement* surface the layer carries; the skill is the first *content* surface. The "first instantiation" question was a false dichotomy — they are different cells of the two-axis table.
- **UQ2 (non-operator skill creation) — RESOLVED: no.** An agent that wants a loop-maintained skill authors a **loop-design entry** (a record, MCP tool, already exists) → the operator decides plan-conversion → the skill is authored via the gated fan-out path when the loop-design's state-2 injection is realized. This **reframes the problem from "manage the skill" to "manage the loop-design as a live requirement"** — validate applicability (drift, via existing `meta_state_derive_status`/`check_grounding`/`query_drift`) + inject its context at the right moment (state-2), with the skill as one realization of that injection. `skill_manage` MCP tool fully dropped (no consumer; no non-operator creates skills). The loop-design (already a 4-kind record, single-writer `meta-state.jsonl`) is the live-requirement layer; the skill is its L3 adapter. No new record kind, no third substrate.
- **UQ3 (mirror dedup) — RESOLVED: (e) write-fanout + (a) parity backstop.** Generalize `core/surfaces.js::writeToAllSurfaces` (verified hardcoded to `coordination/` — `SURFACES.map(s => ${s}/coordination/${subpath})`) to fan out to `<surface>/skills/`; extend the write-gate to `<surface>/skills/**` (gate-monopoly — one MCP write → N identical files, drift prevented by construction). Retain the existing parity test as backstop (detects any gate-bypassing hotfix). Rejected: (b) symlinks (portability fragility — git-on-Windows stores symlinks as text, runtime symlink-following uncertain on WSL2); (c) generate-step (larger touch, justified only as a one-shot migration to materialize the absent `.mastracode/skills/`, not a steady-state mechanism); (d) npm package (wrong consumer model — the three runtimes are sibling surfaces in one repo, discovery is path-bound to `<surface>/skills/`, the threat is drift not versioning; packaging is agentize territory for *external* consumers, wrong here). Mirror scope = SKILL.md + optional scripts only; loop-design entries stay in single-writer `meta-state.jsonl` (no mirroring needed).
- **UQ4 (gate-extension boundary) — RESOLVED: belongs to Rec 12.** This prerequisite gates `<surface>/skills/**` ONLY. Gating `docs/**`/`tools/**`/`core/**` is Rec 12's job (next plan) — a larger detection surface with its own design. The plan states this boundary explicitly so the prerequisite doesn't absorb Rec 12's detection surface.
- **UQ5 (SessionEnd/pre-commit hook) — DEFERRED with a named un-block condition (not an open date).** Manual invocation initially is correct — operator is present, judgment is cheap, and the hook is the bridge to autonomous sessions (the destination, not the current state). The loop cannot *know* hook-readiness until it can measure the manual path's drift rate, which requires the **change-log gap detection query** (the unwired half named in `loop-engine.md` open Q1) + **session-start injection of the gap** into runtime context so a fresh session sees it. Both are folded into a **broadened Rec 12** (trigger + gap detection + session-start gap surfacing — see the 0105 report Addendum 2 update, 2026-07-07). The hook is the *promotion* of the recurring change-log-gap finding: it ships when the broadened Rec 12's gap-rate finding recurs above threshold. **UQ5 un-blocks when the broadened Rec 12 lands** — until then manual invocation is the correct state, and the deferral is conditional, not indefinite. **Plan home (decided 2026-07-07):** (b)+(c) ship in a dedicated follow-on Rec 12-closed-loop plan — not in the lifecycle plan `260706-0958` (which ships only (a), the L1 trigger statement + symmetry) and not in this skill-layer prerequisite.
- **UQ6 (maturity-declaration convention) — RESOLVED: frontmatter `maturity:` field.** One line in the L2 Req #3 generalization: every loop-maintained SKILL.md declares `maturity:` (state-1/2/3) in frontmatter. Detector-friendly, consistent with the two-axis framing, parseable by a future detector without NLP.
- **UQ7 (retroactive generalization of `learning-loop`) — RESOLVED: generalize retroactively.** It already satisfies Req #3; promoting it under the new layer costs little and avoids two skill regimes. Side effect: completing it under the (e) write-fanout materializes the absent `.mastracode/skills/` mirror as part of the same action.

**Net effect on the chosen scope:** items 1–5 (L1/L2/L3 statements, `learning-loop` restructure, write-gate extension, mirror helper, `core/bound-artifacts.js`) all hold. The mirror helper (item 4) is now specifically **(e) write-fanout** (generalize `writeToAllSurfaces` to `skills/`), not "a mirror mechanism." `skill_manage` (deferred) and the third substrate (dropped) stay dropped. The first enforcement surface is the consult-gate the state-2 skill calls. **Only UQ5 (SessionEnd/pre-commit hook) is deferred** — and now with a named un-block (the broadened Rec 12's change-log gap detection + session-start gap surfacing), not an open date; see the UQ5 bullet above.
...
> **Q1, Q2, Q3, Q4, Q6, Q7 + the mirror-dedup question were RESOLVED in the 2026-07-07 consensus session** — see § "Consensus direction" above. Q1 → skill + consult-gate compose (state-2 content + state-3 guardrail). Q2 → no non-operator skill creation; flow is loop-design → operator → skill. Q3 → path stays `<surface>/skills/<name>/SKILL.md`; the absent `.mastracode/skills/` is materialized by the (e) write-fanout; Req #3 discovery paths updated in the L2 generalization. Mirror-dedup → (e) write-fanout + (a) parity backstop. Q4 → gate boundary belongs to Rec 12 (this plan gates `<surface>/skills/**` only). Q6 → frontmatter `maturity:` field. Q7 → generalize `learning-loop` retroactively. **Q5 (SessionEnd/pre-commit hook) deferred with a named un-block** — not open; manual invocation until the broadened Rec 12 (change-log gap detection + session-start gap surfacing) lands, at which point the hook is the promotion of the recurring gap finding. Original Q1–Q7 text retained below for lineage.
