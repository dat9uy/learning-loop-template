...
**Last updated:** 2026-07-01 (Re-sync — Phase E Plan 3 (Housekeeping) + Plan 7 (Stale sweep) + Plan 7 Fix (Corrective) + Plan 8 (Registry drift fix) all ✅ DONE 2026-06-26 via PR #19; Plan 4 (Mastra Code Validation) confirmed ✅ DONE 2026-06-30; all Phase E E1–E6 checkboxes flipped to ✅ DONE; Phase E closed 2026-06-30; remaining: Plan 5 hardening + Phase F + Phase G)
...
## Current State Snapshot (as of 2026-07-01)

| State | Phases / Items |
|-------|----------------|
| **Done** | Phase A (A1–A5) — product-surface re-debate closed 2026-06-13. Phase B (B1–B6) — Bridge 5 codegen engine + LIM-2 fix + loop-design flip closed 2026-06-14. LIM-2 and LIM-7 resolved. Phase C (C1–C7) — Mastra Phase 0-1 (coexistence + deterministic tools + parity gate + cut-over + manifest update) closed 2026-06-17. Phase D (D1–D7) — Mastra Phase 2-3 (workflows + agents + storage + per-agent model config) closed 2026-06-23; Plan 4 (D9 cutover + manifest reconcile + JSON rename) closed 2026-06-24. Phase E Plan 1 (Foundation) — `core/legacy/` → `core/` rename + FCIS doc + schema doc + 3-layer AGENTS.md §1.1 + 7-fingerprint repoint closed 2026-06-25 (PR #15); deny-list bypass follow-up shipped same day (PR #16). Phase E Plan 2 (Interface spec) — 5-req contract + validator + onboarding guide closed 2026-06-25 (PR #17). Phase E Plan 6 (Shell restructure) — shell files moved to `mastra/` subdirectory closed 2026-06-26 (PR #18 squash-merged; merge commit `de01dcc`). Phase E Plan 3 (Housekeeping) — R2 ownership in AGENTS.md §11 + parity-pin label on `workflow-intentional-skip.js` + `docs/legacy-pins.md` + `core/schema-descriptions.yaml` deletion + 2 follow-ups (regression guard SEARCH_PATHS extension + entry #9 `stale → active`) closed 2026-06-26 (PR #19; merge commit `a309b0e`; journal `docs/journals/260626-phase-e-plan-3-housekeeping-shipped.md`). Phase E Plan 7 (Stale sweep) + Plan 7 Fix (Corrective layer) + Plan 8 (Registry drift fix) — registry hygiene arc (14-entry sweep + 10-op corrective batch + sweep-success assertion + `consistencyCheck` probe + 3 audit-trail orphans recovered + write-gate extended to block Write/Edit on `meta-state.jsonl` + 4 review items closed) closed 2026-06-26 (PR #19; journal `docs/journals/260626-phase-e-plan-7-stale-sweep-shipped.md` extended with Plan 7 Fix + Plan 8 sections; diagnostic `plans/reports/diagnostic-260626-1734-phase-e-registry-drift.md`). Phase E Plan 4 (Mastra Code Validation) — MCP-only integration + `.mastracode/` config (mcp + hooks + settings + database) + 7-req contract (5 base + Req #6 hook-declarative-config + Req #7 settings-no-bypass) + 44 MCP tools round-trip verified closed 2026-06-30 (journal `docs/journals/260630-phase-e-plan-4-shipped.md`). **Phase E closed 2026-06-30.** |
| **Open** | **Phase E closed 2026-06-30.** Phase F — Bridge 7 (product-surface binding). Phase G — Skill Migration Track (`ck:*` → MCP tools). |
...
| **Next-up / Hardening** | LIM-3 (caller identity primitive — `RUNTIME_ID` env var), LIM-4 (path traversal, security priority), LIM-5 (test harness), LIM-6 (idempotency cache + silent gate-log), LIM-8 (3 workflow tool passthroughs). LIM-9 (`meta_state_batch` passthrough) RESOLVED 2026-06-26 by PR #16. **Plan 5 (bundled hardening: LIM-3 + R2 write-gate + LIM-4) is now unblocked since Phase E closed 2026-06-30.** |

**Recommended next move:** Plan 5 (bundled hardening: LIM-3 + R2 write-gate + LIM-4) — security-priority items now unblocked since Phase E closed 2026-06-30. Phase F (Bridge 7) and Phase G (Skill Migration) are the next strategic phases. Phase D Plan 4 followups (D-8 cutover follow-through + D-10 F4 gate-bypass resolution + D-13 F4 PR security note) are still 🟡 READY if immediate content work is desired.
...
| LIM-9 | `meta_state_batch` update op at `meta-state-batch-tool.js:17` still uses `.passthrough()` — `Object.assign` at line 483 accepts arbitrary keys | Hardening (next-up) | **Resolved 2026-06-26** via PR #16 (deny-list moved to `core/meta-state.js` + batch handler consults it + 2 regression tests) | Passthrough-removal follow-up |
...
- **Hardening track (5 LIMs after 2026-06-26)**: LIM-3 (caller identity), LIM-4 (path traversal), LIM-5 (test harness), LIM-6 (idempotency cache + silent gate-log), LIM-8 (3 workflow tool passthroughs). **LIM-9 (`meta_state_batch` passthrough) RESOLVED 2026-06-26 by PR #16.** These are security/quality issues that deserve a dedicated audit pass, not a codegen session. Confirmed "next-up" by the operator on 2026-06-14 — not blocking Phase B. File as separate `meta_state_report` findings post-Phase B; LIM-4 is the security priority (CVE-shape), batch the rest into a single audit-trail pass. The 5 remaining LIMs ship together as **Plan 5 (bundled hardening)** — now unblocked since Phase E closed 2026-06-30.
...
## Phase E — Mastra Phase 4-5 (cut over + embed in Mastra Code Mode 1) — **CLOSED 2026-06-30**

**Bucket:** replace the legacy `learning-loop-mcp` server with the Mastra-based one. Embed in Mastra Code via Mode 1 (peer MCP servers) per Q6 / §8. Hook layer: confirm no Mode 1 changes (per §3.9). **Phase E closed 2026-06-30** with the ship of Plan 4 (Mastra Code Validation). Plans 1, 2, 3, 4, 6 + registry hygiene arc (Plans 7/7-Fix/8) all shipped; E1–E6 checkboxes ✅ DONE; E7 ⚪ DEFERRED (= D-12 Mode 2 decision).

- [x] **E1** Replace the legacy `learning-loop-mcp` server with the Mastra-based one (the "cut over" decision). **Closed 2026-06-24 via `plans/260624-1111-phase-d-plan-4-cutover/phase-07-legacy-cleanup-c9.md`** (legacy/ move + MCP server key rename).
- [x] **E2** Mark the old server `legacy` for one release. **Closed 2026-06-24 via `plans/260624-1111-phase-d-plan-4-cutover/phase-07-legacy-cleanup-c9.md`** (tools/ moved to tools/legacy/; core/ + scout/ moved to core/legacy/ + scout/legacy/; #mcp/* alias deleted).
- [x] **E3** Update `.claude/skills/learning-loop/SKILL.md` + `.factory/skills/learning-loop/SKILL.md` to point at the new tool surface. **Closed 2026-06-25 via `plans/260625-1618-phase-e-interface-spec/`** (E.0 work bundled in Plan 2: SKILL.md files reference new contract + `loop_describe` + `meta_state_list` + `interface/CONTRACT.md`).
- [x] **E4** Update `agent-manifest.json` to the new group names. **Closed 2026-06-24 via `plans/260623-1619-phase-d-plan-3-agents/` + `plans/260624-1111-phase-d-plan-4-cutover/phase-02-manifest-reconciliation.md`** (6-group structure with 44 tools; `mastra_`-prefixed deterministic subset + agents).
- [x] **E5** Mode 1: Mastra Code connects via MCP to the loop's `MCPServer`. **Closed 2026-06-30 via `plans/260630-2012-phase-e-plan-4-mastra-code-validation/`** (MCP-only integration; `.mastracode/` configDir via `bootLocalAgentController`; 7-req contract: 5 base + Req #6 hook-declarative-config + Req #7 settings-no-bypass; 44 MCP tools round-trip verified; journal `docs/journals/260630-phase-e-plan-4-shipped.md`).
- [x] **E6** Hook layer: confirm no Mode 1 changes (per §3.9). **Closed 2026-06-30 via `plans/260630-2012-phase-e-plan-4-mastra-code-validation/`** (no hook-layer changes needed for Mode 1 peer-MCP; Mastra Code hook surface documented in `docs/agents/mastra-code.md`; declarative-config Req #6 + settings-no-bypass Req #7 added to the contract).
...
### Phase E — Refined plan split (per scope report Rev 12 — **Phase E closed 2026-06-30**)

**Origin:** the scope report at `plans/reports/phase-e-scope-260624-2025-runtime-interface-structure-report.md` (Rev 12) re-scoped Phase E into 6 plans (1–6) + 1 deferred hardening plan (5). Plans 1, 2, 3, 4, 6 all shipped; Plan 5 (bundled hardening) is the only remaining Phase E item. **The E1–E7 checkboxes above remain the canonical tracker view** (E1–E6 closed; E7 deferred); the refined split below is the **execution history** for the shipped plans.
...
| **2** | **phase-e-interface-spec** | E.0 (skill spec updates) + E.1b (new `interface/` dir with 5-requirement contract, validator, onboarding guide). **The new spec + the docs that point to it.** | ~1.25d | Plan 1 (FCIS invariant) | ✅ **DONE 2026-06-25** (`plans/260625-1618-phase-e-interface-spec/`, PR #17) |
| **3** | **phase-e-housekeeping** | E.2 (`AGENTS.md §11` R2 ownership) + E.3 (parity-pin label + `docs/legacy-pins.md`) + E.4 (delete or rewrite `core/legacy/schema-descriptions.yaml`). | ~1.5h | Plan 1 (FCIS invariant) — can run in parallel with Plan 2 | ✅ **DONE 2026-06-26** (`plans/260626-0607-phase-e-housekeeping/`, PR #19 merge commit `a309b0e`; journal `docs/journals/260626-phase-e-plan-3-housekeeping-shipped.md`; 2 follow-ups from Plan 6 review bundled: regression guard extension + entry #9 `stale → active`) |
| **4** | **phase-e-mastra-code-validation** | E.5 — smoke-test `createMastraCode({ cwd })` (MCP-only integration); satisfy 7 contract requirements for Mastra Code (5 base + Req #6 hook-declarative-config + Req #7 settings-no-bypass); document in `docs/agents/mastra-code.md`. **The worked example that proves the onboarding flow works.** | 1–2d | Plan 2 + Plan 6 (contract args path must be post-move) | ✅ **DONE 2026-06-30** (`plans/260630-2012-phase-e-plan-4-mastra-code-validation/`, journal `docs/journals/260630-phase-e-plan-4-shipped.md`) |
...
| **6** | **phase-e-shell-restructure** | E.6 — move shell files (server.js, create-loop-*.js, legacy-handler-adapter.js, schema-parity.js, schemas.js, workflows/, agents/, +2 manifests) from top-level of `tools/learning-loop-mastra/` into `mastra/` subdirectory. Makes Layer 2 (Mastra shell) physically first-class. | 1–1.5d | Plan 1 + Plan 2 | ✅ **DONE 2026-06-26** (`plans/260626-0302-phase-e-shell-restructure/`, branch `phase-e/plan-6-shell-restructure`) |

**Total Phase E (Plans 1–4) effort:** ~4–5 days. **Total with hardening (Plans 1–5):** ~3–4 weeks.

**Registry hygiene plans shipped during Phase E (2026-06-26 via PR #19):**

Three additional plans shipped during Phase E that are technically registry maintenance (not Phase E *content*) but were prerequisites for Phase E closure's audit-trail integrity:

| # | Plan | Scope | Effort | Status |
|---|------|-------|--------|--------|
| **7** | **phase-e-stale-sweep** | Sweep 14 stale `mechanism_check=true` entries → `status: active` via 2 atomic `meta_state_batch` invocations. Refresh 1 drifted fingerprint. Add cold-tier regression test for sweep success. Closes the TTL recursion gap. | ~1d | ✅ DONE 2026-06-26 (PR #19; journal `docs/journals/260626-phase-e-plan-7-stale-sweep-shipped.md`) |
| **7-fix** | **phase-e-stale-sweep-fix** | 5-phase corrective layer: (1) 10-op corrective `meta_state_ack` batch; (2) sweep-success assertion; (3) audit-gap investigation for the unlogged write path; (4) doc correction (root cause was `checkStaleness`, not `expires_at`); (5) verify + commit. | ~1d | ✅ DONE 2026-06-26 (PR #19; journal extended with Plan 7 Fix section) |
| **8** | **phase-e-registry-drift-fix** | (a) `consistencyCheck(entries)` pure function + 16 unit tests; (b) `meta_state_consistency_check` MCP probe + 8 tool tests; (c) diagnose 3 audit-trail orphans; (d) fix via canonical MCP path; (e) write-gate extended to block Write/Edit on `meta-state.jsonl`. Closes the audit-log gap. | ~1.5d | ✅ DONE 2026-06-26 (PR #19; diagnostic `plans/reports/diagnostic-260626-1734-phase-e-registry-drift.md`) |
...
**Bucket:** the end-state phase. Phase B–E have all shipped (2026-06-30) and the loop has meta-surface substrate to reason with. The Phase A re-debate conclusions feed this phase. Phase F is the next strategic content phase after Phase E closure; can be planned in parallel with Plan 5 (hardening) since they touch different surfaces (Bridge 7 = schema binding; hardening = identity primitives).
...
| ID | Task | Severity | Status | Source |
|----|------|----------|--------|--------|
| E1 | Replace legacy `learning-loop-mcp` server with Mastra-based one | high | ✅ DONE (2026-06-24, Phase D Plan 4) | `plans/260624-1111-phase-d-plan-4-cutover/phase-07-legacy-cleanup-c9.md` (legacy/ move) + phase-08 (MCP server key rename) |
| E2 | Mark old server `legacy` | high | ✅ DONE (2026-06-24, Phase D Plan 4) | `plans/260624-1111-phase-d-plan-4-cutover/phase-07-legacy-cleanup-c9.md` (tools/ moved to tools/legacy/; core/ + scout/ moved to core/legacy/ + scout/legacy/; #mcp/* alias deleted) |
| E3 | Update SKILL.md files | high | ✅ DONE (2026-06-25, Phase E Plan 2 / E.0) | `plans/260625-1618-phase-e-interface-spec/` (SKILL.md references new contract + `loop_describe` + `meta_state_list` + `interface/CONTRACT.md`) |
| E4 | Update `agent-manifest.json` | high | ✅ DONE (2026-06-24, Phase D Plan 3 + Plan 4) | `plans/260623-1619-phase-d-plan-3-agents/` (6th group added) + `plans/260624-1111-phase-d-plan-4-cutover/phase-02-manifest-reconciliation.md` (44-tool total) |
| E5 | Mode 1: Mastra Code connects via MCP | high | ✅ DONE (2026-06-30, Phase E Plan 4) | `plans/260630-2012-phase-e-plan-4-mastra-code-validation/` (7-req contract; 44 tools round-trip; journal `docs/journals/260630-phase-e-plan-4-shipped.md`) |
| E6 | Hook layer: confirm no Mode 1 changes | high | ✅ DONE (2026-06-30, Phase E Plan 4) | `plans/260630-2012-phase-e-plan-4-mastra-code-validation/` (no hook-layer changes for Mode 1; `docs/agents/mastra-code.md` documents the Mastra Code hook surface; Req #6 + Req #7 added) |
...
### Phase E — Refined plan split (per scope report Rev 12 — Phase E closed)

| ID | Task | Severity | Status | Source |
|----|------|----------|--------|--------|
| PE-1 | **Plan 1 — Foundation**: rename `core/legacy/` → `core/` + `core/README.md` (FCIS invariant) + `docs/schemas.md` + `AGENTS.md §1.1` (3 layers). | high | ✅ DONE (2026-06-25) | `plans/260624-2335-phase-e-foundation/` (PR #15) + PR #16 deny-list follow-up |
| PE-2 | **Plan 2 — Interface spec**: E.0 (skill spec updates) + E.1b (new `interface/` dir with 5-requirement contract, validator, onboarding guide). | high | ✅ DONE (2026-06-25) | `plans/260625-1618-phase-e-interface-spec/` (PR #17; journal `docs/journals/260625-phase-e-plan-2-interface-spec-shipped.md`) |
| PE-3 | **Plan 3 — Housekeeping**: E.2 (R2 ownership) + E.3 (parity-pin label) + E.4 (schema rot cleanup) + 2 Plan 6 follow-ups. | medium | ✅ DONE (2026-06-26, PR #19) | `plans/260626-0607-phase-e-housekeeping/` (journal `docs/journals/260626-phase-e-plan-3-housekeeping-shipped.md`) |
| PE-4 | **Plan 4 — Mastra Code validation**: smoke-test `createMastraCode({ cwd })` (MCP-only integration); satisfy 7 contract requirements; document in `docs/agents/mastra-code.md`. | high | ✅ DONE (2026-06-30) | `plans/260630-2012-phase-e-plan-4-mastra-code-validation/` (journal `docs/journals/260630-phase-e-plan-4-shipped.md`) |
| PE-5 | **Plan 5 — Hardening** (LIM-3 + R2 write-gate + LIM-4): `RUNTIME_ID` env var convention; runtime-marker reader; per-runtime write allowlist keyed on `RUNTIME_ID`; path traversal fix in `meta_state_refresh_fingerprint`. | high (security) | 🔵 OPEN (**now unblocked** since Phase E closed 2026-06-30) | scope report § "Plan split for execution" row 5; bundled per user "Bundle" decision |
| PE-6 | **Plan 6 — Shell restructure**: move shell files from top-level of `tools/learning-loop-mastra/` into `mastra/` subdirectory. | high | ✅ DONE (2026-06-26) | `plans/260626-0302-phase-e-shell-restructure/` (PR #18 squash-merged at `de01dcc`; journal `docs/journals/260626-phase-e-plan-6-shell-restructure-shipped.md`) |
...
| D-19 | LIM hardening — LIM-3 (caller identity), LIM-4 (path traversal, security priority), LIM-5 (test harness), LIM-6 (idempotency cache + silent gate-log), LIM-8 (3 workflow tool passthroughs); LIM-9 (`meta_state_batch` passthrough) RESOLVED 2026-06-26 by PR #16 — remove from this list when planning | high (security) | 🔵 OPEN (5/6 LIMs remain; bundled into Plan 5) | D-19; see LIM table at line ~116 |
...
| PE-2 | Phase E Plan 2 (Interface spec): `interface/` dir with 5-req CONTRACT.md + contract.js validator + RUNTIME_ONBOARDING.md + 24 contract tests + 5 regression guards; SKILL.md updates in `.claude/` + `.factory/` | `plans/260625-1618-phase-e-interface-spec/` (PR #17) — 2026-06-25; journal `docs/journals/260625-phase-e-plan-2-interface-spec-shipped.md`; 18-finding red-team review → 11 applied + 7 accepted |
| PE-3 | Phase E Plan 3 (Housekeeping): `AGENTS.md §11` R2 ownership + `legacy-pins.md` parity-pin convention + `core/schema-descriptions.yaml` deletion + regression guard extension + entry #9 `stale → active` corrective | `plans/260626-0607-phase-e-housekeeping/` (PR #19, merge commit `a309b0e`) — 2026-06-26; journal `docs/journals/260626-phase-e-plan-3-housekeeping-shipped.md`; 2 critical blockers caught and resolved post-review (Rev 9) |
| PE-6 | Phase E Plan 6 (Shell restructure): move 11 shell file-groups from top-level of `tools/learning-loop-mastra/` into `mastra/` subdirectory + ~31 external path refs + 6 regression guards + 9 meta-state entries repointed | `plans/260626-0302-phase-e-shell-restructure/` (PR #18 squash-merged at `de01dcc`) — 2026-06-26; journal `docs/journals/260626-phase-e-plan-6-shell-restructure-shipped.md`; code review APPROVE WITH FOLLOW-UPS |
| PE-7 | Phase E Plan 7 (Stale sweep): 14-entry sweep via 2 atomic `meta_state_batch` calls + 1 drifted fingerprint refresh + cold-tier regression test | `plans/260626-0720-phase-e-stale-sweep/` (PR #19) — 2026-06-26; journal `docs/journals/260626-phase-e-plan-7-stale-sweep-shipped.md` |
| PE-7-FIX | Phase E Plan 7 Fix (Corrective layer): 10-op corrective `meta_state_ack` batch + sweep-success assertion + audit-gap investigation + root-cause doc correction | `plans/260626-1535-phase-e-stale-sweep-fix/` (PR #19) — 2026-06-26; journal extended with Plan 7 Fix section |
| PE-8 | Phase E Plan 8 (Registry drift fix): `consistencyCheck` pure function + 16 unit tests + `meta_state_consistency_check` MCP probe + 3 audit-trail orphans recovered (O-1, O-2, O-3) + write-gate extension to block Write/Edit on `meta-state.jsonl` | `plans/260626-1734-phase-e-registry-drift-fix/` (PR #19) — 2026-06-26; diagnostic `plans/reports/diagnostic-260626-1734-phase-e-registry-drift.md` |
| PE-4 | Phase E Plan 4 (Mastra Code Validation): MCP-only integration + `.mastracode/` config (mcp + hooks + settings + database) + 7-req contract (5 base + Req #6 hook-declarative-config + Req #7 settings-no-bypass) + 44 MCP tools round-trip verified | `plans/260630-2012-phase-e-plan-4-mastra-code-validation/` — 2026-06-30; journal `docs/journals/260630-phase-e-plan-4-shipped.md`; 3 prep-report corrections discovered live (tool namespacing `learning-loop_mastra_<tool>`, API `bootLocalAgentController`, McpManager `init()`/`getTools()`) |

**Total deferred items:** 18 (3 Phase C continuation ready, 2 Phase D open, 1 Phase E original [E7 = D-12], 1 Phase E refined [PE-5 hardening], 5 hardening, 4 Phase G, 3 cross-cutting). 3 of 3 Phase C continuation items are 🟡 READY (D-8, D-10, D-13); 0 Phase C items open. **Phase E closed 2026-06-30**; remaining Phase E items: E7 (⚪ DEFERRED) + PE-5 (🔵 OPEN, now unblocked).
...
- **Not a brainstorm.** Phase A is *closed* (A1-A5 flipped `[x]` 2026-06-13). Phases B–E are *closed* (Phase E closed 2026-06-30 with the ship of Plan 4). Phases F + G + Plan 5 hardening remain as the next work; the tracker surfaces that state, not the design.
