...
| **Open** | Phase E — Plan 3 (Housekeeping: R2 ownership + parity-pin label + schema rot cleanup); Plan 4 (Mastra Code validation, unblocked by Plan 6). Phase F — Bridge 7 (product-surface binding). Phase G — Skill Migration Track (`ck:*` → MCP tools). |
...
| **4** | **phase-e-mastra-code-validation** | E.5 — smoke-test `createMastraCode({ configDir })` against the new MCPServer; satisfy 5 contract requirements for Mastra Code; document in `docs/agents/mastra-code.md`. **The worked example that proves the onboarding flow works.** | 1–2d | Plan 2 + Plan 6 (contract args path must be post-move) | 🔵 OPEN (depends on Plan 2 + Plan 6) |
