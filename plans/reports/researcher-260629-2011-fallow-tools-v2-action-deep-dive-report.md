...
**§6.3 Multi-run SARIF — CORRECTED.** Fallow's SARIF output contains multiple runs (one per analyzer: dead-code, health, dupes). This was the original problem that motivated the Python split heredoc in our current workflow (CI audit §1b, §3d, §3e). The Action's solution is the **single `category: fallow`** mapping — but the original claim that "`codeql-action/upload-sarif@v4` accepts multi-run SARIF when they share a category" is **false**. The `areAllRunsUnique` validator in `github/codeql-action/src/sarif/index.ts` builds its `createRunKey` from `run.tool?.driver?.{name,fullName,version,semanticVersion,guid}` + `run.automationDetails?.id` — `category` is NOT a key field (verified at `plans/reports/research-260630-1425-GH-2011-fallow-sarif-internals-audit.md` § Layer 1; plan 260630-0536 §"Why the deep-dive was wrong"). Fallow's multi-run SARIF runs collide on `createRunKey` because they share `driver.name="fallow"` + `driver.version` from `env!("CARGO_PKG_VERSION")` and the dead-code/health builders do not set `automationDetails.id`. The GitHub 2025-07-21 changelog tightened `areAllRunsUnique` (pre-change, multi-run collisions were silently collapsed); the validator's rejection message mentions "same category" but the validator itself does not key on category. **The correct fix** (plan 260630-0536 §"Corrected design") is an inline jq patch that rewrites `runs[i].automationDetails.id` per run so `createRunKey` produces unique values, then a single explicit `codeql-action/upload-sarif@v4` upload call. The trade-off remains: loss of per-analyzer category grouping in the Code Scanning UI.
...
**§6.5 `sarif: true` does both — CORRECTED.** Setting `sarif: 'true'` (the original Phase 4 contract) triggers both local SARIF generation (`analyze.sh` fallback block) AND Code Scanning upload (`Upload SARIF` step's `if:`). One input, two effects. **However, the upload step rejects fallow's multi-run SARIF** (see §6.3 corrected): the Action writes the file but `codeql-action/upload-sarif@v4`'s `areAllRunsUnique` validator fails because the dead-code and health runs collide on `createRunKey`. Fallow's `fallow audit` emits 2-3 runs depending on findings (dead-code always; dupes only if non-empty `clone_groups`; health only if findings cross threshold). All runs share `tool.driver.name = "fallow"` and `driver.version` from `env!("CARGO_PKG_VERSION")`; none set `fullName`, `guid`, `semanticVersion`, or `automationDetails` (verified at `plans/reports/research-260630-1425-GH-2011-fallow-sarif-internals-audit.md` § Layer 2). The dupes run is the partial-fix exception: `crates/api/src/audit_output.rs::build_audit_duplication_sarif_run` synthesizes it locally with `automationDetails.id = "fallow/audit/dupes"`. Dead-code and health runs are spread verbatim from upstream builders that do not set `automationDetails`, so they collide on `createRunKey` (verified against `tools/learning-loop-mastra/reports/fallow/audit.sarif`: runs[0]=dead-code automationDetails=null, runs[1]=dupes automationDetails.id="fallow/audit/dupes", runs[2]=health automationDetails=null — runs 0 & 2 collide). The 2-run case (no dupes findings) shows runs[0] and runs[1] both with automationDetails=null — same collision. **The recovery path** (plan 260630-0536 §"Corrected design") flips `sarif: false` on the Action, adds an inline jq patch step that rewrites `automationDetails.id` per run, then uploads the patched file with 1 explicit `codeql-action/upload-sarif@v4` call. [resolved with amendment]
