...
## 8. W plan status (2026-07-22, post-plan-draft + operator confirmation)

The W plan is drafted at `plans/260722-1343-write-capable-cli-w-complete-the-cli-record-transport/` (4 phases, TDD, red-team-reviewed). Operator confirmed at plan time:

- **T2 read-path evidence: accrued** — W's Phase 1 may start (Preconditions gate met).
- **Tool-set boundary (Q1): all 16 mutation handlers** ride the CLI (`meta_state_promote_rule` post-guard; `meta_state_dispatch_finding` both stages). Auxiliary read-ish tools stay MCP for now — see follow-up below.
- **Self-footgun (Q5): guard added** — a promotion-path self-match guard (regex matching CLI invocation shapes: relative + absolute + bare) lets `meta_state_promote_rule` ride the CLI. The existing `cli-self-footgun-guard.test.js` lock flips to assert rejection.
- **Opt-out flag (new decision): one combined flag** `LOOP_RECORDS_VIA_CLI` drops the full `CLI_TOOLS` set (reads + writes) from MCP. R's `LOOP_READS_VIA_CLI` stays for reads-only backward compat; `.claude` migrates to the combined flag at Phase 4 (after write-path T2). This supersedes the two-flag default floated in §3.1.
- **Q2 correction:** the §7 Q2 recommendation ("`commit` stays MCP — avoid `gh`/network") was based on a misread. Verified in `meta-state-dispatch-finding-tool.js`: `handleCommitStage` does NOT call `gh` — it only does `appendLedgerEvent` + `updateEntry`; the *agent* runs `gh issue create` between `prepare` and `commit` (handler comment line 14). So `commit` is CLI-portable by W's own criterion; both stages ride the CLI.
- **Red-team fixes applied:** the plan's Phase 4 dogfood was reordered to dogfood-with-MCP-fallback before any flag flip (chicken-and-egg C1); the guard insertion point corrected (activation branch, not "after `isSafeRegexPattern`" which is preview-only — C5); `CLI_WRITE_TOOLS` enumerated with a drift test (C3); exit 1 split into recognized-rejection vs `InternalError` (H1); write-parity comparator expanded to `gate-log.jsonl` + `runtime-state.jsonl` with a write-specific strip set (H2).

### Follow-up: auxiliary read-ish tools (deferred from W)

W closes the split-brain for the **record surface** (reads + mutation handlers). The auxiliary read-ish handler tools — `gate_check`, `gate_check_recurrence`, `meta_state_sweep`, `meta_state_query_drift`, `meta_state_relationship_validate` — are neither in the 7 reads nor mutation handlers, so leaving them on MCP does not re-open the split. They stay MCP in W. A **follow-up** (separate plan, not W) may add them to `CLI_READ_TOOLS` so a runtime that sets `LOOP_RECORDS_VIA_CLI=1` gets the full record + gate + drift surface on the CLI and MCP keeps only the irreducible residue (workflow registry, storage, `update_r2_allowlist`, `check_runtime_agnostic`). That follow-up should not start until W's write-path T2 evidence accrues and `.claude` has migrated to the combined flag.