...
- **Post-review correction (e2e count):** the "19 connectMcpServer + 6 spawnSync = 25" figure was unreconciled — it double-counted spawnSync files that also use `connectMcpServer` and missed the standalone CLI-spawn set. The accurate breakdown after the code-review fix: **28 e2e files = 23 MCP-marker files (`connectMcpServer`/`with-mcp-server`/`StdioClientTransport`/`@modelcontextprotocol/sdk/client`) + 5 CLI-spawn files (`LOOP_BIN`/`cliPath` — `spawnSync`/`spawn` of `bin/loop.mjs`)**. The original guard marker pattern only covered MCP-server spawns; the CLI-spawn half was added in the review fix (see Deviations).
...
A `code-review` pass on the shipped commits surfaced three defects; all fixed on this branch:

1. **CLI-spawn tests misclassified as unit (C1).** The guard marker pattern only covered MCP-server spawns; 5 files that `spawnSync`/`spawn` the `bin/loop.mjs` CLI binary (the plan's *other* e2e criterion) sat in `unit` and ran on every pre-commit. Fixed by extending `MARKER_PATTERN` with `LOOP_BIN|cliPath` and adding the 5 files to `E2E_FILES` (23 → 28). The guard now catches CLI-spawn drift, not just MCP-server drift.
2. **`fallow:gate` broken on `-50` coverage parse (H1).** `@vitest/coverage-istanbul` emits negative branch hit counters (e.g. `-50`) that fallow's u32 parser rejects (`invalid value: integer -50, expected u32`), making the pre-push hook fail with exit 2 on every push. The original report framed this as "pre-existing on main"; re-verification showed that framing was **wrong** — the `-50` is config-shape-dependent (it appears under the prior per-project coverage config, not under root coverage, which `main` uses). Root cause was the per-project coverage design plus `sanitize-coverage.mjs` only clamping `column`/`line` (not hit counters). Fixed two layers: coverage moved to root (eliminates `-50` at the source — 0 negatives under root coverage on both full and e2e-only runs) and the sanitize script now also clamps negative `b`/`s`/`f` hit counters to 0 (defense-in-depth for partial-coverage paths). `fallow:gate` now exits 1 (real findings) instead of 2 (parse failure).
3. **Coverage regression surfaced by H1 (M2).** The original coverage shape (coverage only on the `e2e` project) meant unit-exercised source files got no instrumentation, so `fallow:gate` flagged them as 0%-tested with inflated CRAP. This was "accepted" only because the broken `-50` parse hid it. Fixed by moving coverage back to the root (so `pnpm test` instruments ALL source files) and disabling it in `test:unit` via the `--coverage.enabled=false` CLI flag (verified empirically to override root config, unlike the per-project quirk). Pre-commit stays fast; pre-push now produces full coverage for fallow.
