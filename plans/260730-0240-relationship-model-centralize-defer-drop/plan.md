...
| 3 | Write-time emits a **warn-only** structural-RI advisory (gate-log naming the dangling `{field, id}`) — never rejects — for cross-ref fields whose target id is **never-existent** at `writeEntry`/`updateEntry`/`metaStateBatch` (new-appends + update-changed-only; id-existence only — tombstones count as present, kind-match + liveness out of scope). CI `meta-state-refs-check.yml` is the hard enforcer. `applies_to_resolution` exempt (not an entry-id ref). `tryClaimSessionId` gets a defensive comment, not RI (test-only path, bypasses `writeEntry`). Historical entries still read fine | P1 |
...
|---|-------|--------|----------|--------|------|
| 1 | [Characterization tests + bug red-tests](./phase-01-start.md) | Completed | P1 | 2h | — |
| 2 | [Core relationship-graph module](./phase-02-relationship-graph-module.md) | Completed | P1 | 3h | 1 |
| 3 | [Migrate consumers + preserve reopens symmetry](./phase-03-migrate-consumers-reopens-symmetry.md) | Completed | P1 | 4.5h | 2 |
| 4 | [Write-time structural RI validation](./phase-04-write-time-structural-ri.md) | Completed | P1 | 3h | 2,3 |
| 5 | [Document three-mechanism boundary](./phase-05-document-three-mechanism-boundary.md) | Completed | P2 | 1.5h | 3 |
| 6 | [Resolve findings + YAGNI deferral record](./phase-06-resolve-findings-yagni-deferral.md) | Completed | P2 | 1h | 3,4,5 |
| 7 | [Verify + runtime-agnostic audit](./phase-07-verify-runtime-agnostic-audit.md) | Completed | P2 | 1h | 3,4,5,6 |
...
core/meta-state.js   ← MODIFY (write-time RI): writeEntry (:1115) / updateEntry (:1189) / metaStateBatch (:1507)
  emit a WARN-ONLY structural-RI advisory (appendGateLog) — never reject (CI meta-state-refs-check.yml is the hard enforcer); existence set = readRegistry(root) inside withRegistryLock
  (in-batch `inBatchIds` accumulator for metaStateBatch so intra-batch write→reference passes). ⚠️ ID-EXISTENCE ONLY (tombstones count as present — narrowed from "dangling"); updateEntry
  accepts the patch (returns `true`) + logs the advisory — the "dangling_structural_ref" string code was REMOVED under warn-only;
...
- [x] One core module (`core/entry/relationship-graph.js`) is the single source for (a) cross-ref fields per kind, (b) forward+inverse resolution, (c) write-time structural RI (id-existence) — consumed by the factories, `loop-introspect`, the relationships tool, the validate tool, and the CI validator (no bespoke per-kind `outboundRefs`/`inboundRefs` duplication). The wire shape + `computeDanglingRefs` stay in the relationships tool (presentation + needs `stale-view`)
- [x] The 3 forward + 2 inverse implementations collapse to one; the validator's `loop-design` kind-`"meta"` bug and omitted rule `supersedes`/`applies_to_resolution` are fixed; `consolidates-refs.js` + `inbound-from-loop-design.js` kept as thin re-exports (rollback-safe)
- [x] Write-time emits a **warn-only** structural-RI advisory (gate-log naming the dangling `{field, id}`) — never rejects — for cross-ref fields whose target id is **never-existent** at `writeEntry`/`updateEntry`/`metaStateBatch` (id-existence only — tombstones count as present; kind-match + liveness deferred; `applies_to_resolution` exempt → no advisory); `updateEntry` accepts the patch (returns `true`) — the `"dangling_structural_ref"` string code was REMOVED under warn-only; covered by a test (accept + advisory-logged assertions); new-appends + update-changed-only; `tryClaimSessionId` gets a defensive comment (no RI); historical entries read fine; CI `meta-state-refs-check.yml` is the hard enforcer
- [x] `meta_state_relationships({id, direction:"outbound"})` on a child with `reopens` returns `outbound.reopens` populated AND `inbound.reopened_by` populated (regression-prevention test); legacy findings without `promoted_to_rule` keep `outbound.promoted_to_rule` via the targeted `inverseRefs` fallback (snapshot test :77-78 preserved); the 4 existing `reopens` edges still read fine
- [x] The dual-field `promoted_to_rule_inverse` is 1 ref (canonical `rule.origin`); `build-inverse-indexes.test.js:67` + the relationships-snapshot fallback test + the `computeTopReferences`/`top_references` characterization updated in the same change; legacy `finding.promoted_to_rule` data preserved on disk
- [x] The cold-tier cache gets a deploy-step bust (delete the cache file OR bump a cache-version constant) so a code-only deploy doesn't serve stale 2-ref indexes; `top_references` warm/cold tiers agree after deploy
- [x] Boundary documented in `docs/meta-state-lifecycle.md` + `docs/architecture.md` + a docs note (not a session-start hint); no `related_to` field; stale tool descriptions corrected
- [x] `check_runtime_agnostic` passes on the new module (runtime-agnostic.test.js 22 + check-runtime-agnostic-tool 5 green); **full suite green** (291 passed, 0 failed — the 16-file hard-reject regression fixed by the warn-only reframe); no `reopens`/`cascade_from` public-contract regression
- [x] `meta-260623T1126Z` + `meta-260715T2237Z` + `meta-260717T1004Z` resolved/superseded with `reopens`/`source_refs` lineage (resolution note precise about the tool-layer-fixed-out-of-band nuance); the `reopens`/`cascade_from` drop recorded as an observable YAGNI deferral; change-log logged; writes ride `bin/loop.mjs` (the 2 wired runtimes both set `LOOP_RECORDS_VIA_CLI=1`)
...
**Trigger:** Post-implementation review of the last 5 commits found Phase 4 (write-time RI) shipped as a **hard reject**, which regressed the full vitest suite (16 test files failing via `dangling_structural_ref`) and broke two features that deliberately create ref orphans at write time: the `dangling_refs` "missing" view (seeds `reopens` → never-existent id via `writeEntry`) and the cold-tier `orphans` array (`consolidated_into` → missing change-log). Diagnosis proved no hard-reject subset unblocks all 16 — every structural field is orphaned by some test, and the orphan/dangling features *require* the write path to accept never-existent targets.
**Decision:** Flip write-time structural RI to **WARN-ONLY** — emit a gate-log advisory (naming the dangling `{field, id}`) and continue the append; never reject. The CI gate `meta-state-refs-check.yml` stays the hard enforcer (catches within-PR orphans) — consistent with red-team R2's framing that write-time RI's marginal benefit is immediate feedback + cross-PR orphans, not correctness rejection. This supersedes the hard-reject design in Goal #3 / Architecture / Success Criteria #3 and red-team R7's `updateEntry` string-code disposition (the `"dangling_structural_ref"` string code was removed; `updateEntry` keeps its `true`/`null`/`"version_mismatch"` contract).
**Also fixed in this pass:** (a) `writeEntry`'s RI error no longer reads `dangling_structural_ref: []` — the advisory now carries the actual dangling list (was lost because `assertinvariant`'s `returnOnFail` carried no `dangling`); (b) `metaStateBatch` now seeds an `inBatchIds` accumulator from the registry + grows it per write-op (incl. change-logs), so an intra-batch "write change-log then reference it" no longer false-warns; (c) regenerated the `cli-context-savings-script.test.js` snapshot (latent drift from the PR's `f17c465` tool-description change, unmasked by the fix).
**Verification:** `npx vitest run` full suite exit 0 — 291 passed, 1 skipped, 0 failed (was: exit 1, 16 files failing); `dangling_structural_ref` rejection count 0; `reopens`/`cascade_from` contract unchanged; runtime-agnostic tests green.
**Phase 6 reconciliation:** #1/#2 resolved (resolved_at 2026-07-30) with the scout-verified nuance notes; #3 left open + deferral + `last_verified_at` re-stamped; change-log logged. `evidence_code_ref` repointing + `source_refs` + re-verify-before-resolve were intentionally skipped (YAGNI for *resolved* findings — the loop only re-grounds open findings; lineage lives in the resolution notes + change-log). Documented in phase-06.
