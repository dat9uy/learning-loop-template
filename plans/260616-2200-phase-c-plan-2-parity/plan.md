...
**Acceptance gate (the single sentence, durable anchor):** *"All 9 test namespaces pass against `learning-loop-mcp` (durable anchor) AND byte-identical `inputSchema` for all 29 deterministic tools via `z.toJSONSchema()` (F7 + F11 resolved) + byte-identical `tools/call` content for the 5-tool read-only subset (`meta_state_list`, `loop_describe`, `runtime_state_read`, `check_runtime_agnostic`, `gate_check`) via `JSON.parse(content[0].text)` deepEqual + 29 tools enumerated by the parallel mastra cold-session E2E matching `tools/learning-loop-mastra/tools/manifest.json` + the dual-server `tools/list` returns 40 + 29 = 69 distinct names with no collisions."* — **Note:** 24/29 tools are schema-only parity; only 5 are full content parity (write-side tools are excluded from content parity to avoid registry mutation races). The 24/29 split is honest about what the gate proves.
...
- **Test count math (per R-02 + R-07 acceptance + validation):** Plan 1's namespace 10 baseline is 55 tests = 5 (wire-format-coercion-fix) + 6 (wire-format-top-level-coercion) + 5 (wire-format-meta-state-optional-fields) + 4 (wire-format-patch-recursion) + 29 (parity-schema-shape) + 6 (mcp-config-peer). Phase 4 **deletes** the 29-test `parity-schema-shape.test.js` (per validation) and adds `parity-zod-to-json-schema.test.js` with **29 schema tests + 5 read-only `tools/call` tests + 3 invariant probes = 37 tests** (one `test()` block per tool; multiple `assert` calls inside). Net namespace 10 = 55 - 29 + 37 = **63 tests**, NOT 117 as the prior draft claimed. Phase 5 adds 5 cold-session tests. Phase 6 adds 3 collision tests. Total mastra-specific = 63 + 5 + 3 = **71 tests**. The "126" anchor in the prior draft is wrong; the correct count is **71 mastra-specific + 9 legacy namespaces = 80 distinct test scopes** (the 9-namespace anchor is durable; per-test counts drift).
