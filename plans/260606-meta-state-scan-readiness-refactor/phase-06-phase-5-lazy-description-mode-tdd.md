...
Adds a `description_mode` enum to `loop_describe` schema. Cold tier defaults to `'summary'`; warm/hot tiers default to `'full'`. Summary mode returns a 200-char preview of the description plus the relationship-relevant fields. Full mode returns the entry as today. Cold-tier token cost drops from ~27K to ~8K in summary mode.

## Requirements

- **Functional**: `loop_describe({ tier: 'cold' })` (no flag) returns descriptions truncated to 200 chars; `loop_describe({ tier: 'cold', description_mode: 'full' })` returns full descriptions.
- **Non-functional**: cold-tier token count in summary mode ≤ 16K (down from 27K); `summarize(entry)` is a pure function in `core/loop-introspect.js` (reusable for Phase 7's `registry_summary` field); the schema is opt-in (existing callers see no diff).

## Architecture

A pure function `summarize(entry)` in `core/loop-introspect.js` projects an entry to its summary shape:
- `id`, `entry_kind`, `status`, refs, `description_preview: entry.description?.slice(0, 200) + (entry.description?.length > 200 ? '...' : '')`.

The `loop_describe` tool's schema gains `description_mode: z.enum(['summary', 'full']).optional()`. The cold tier branch uses `mode ?? 'summary'` (default for cold); warm/hot use `mode ?? 'full'`.

```
loop_describe({ tier: 'cold', description_mode: 'summary' })
  → entries = readRegistry(root) + filter
  → summary = entries.map(summarize)
  → return { tier: 'cold', entries: summary, ... }
...
2. In `__tests__/loop-describe-description-mode.test.js`, write 3 test cases:
   - `test('cold tier with no description_mode returns summary mode (default)', ...)` — call with `tier: 'cold'`; expected: response has `description_mode: 'summary'` and entries have `description_preview`.
   - `test('cold tier with description_mode: full returns full descriptions', ...)` — call with `tier: 'cold', description_mode: 'full'`; expected: entries have full `description` (no `description_preview`).
...
6. Edit `tools/loop-describe-tool.js`:
   ...
   - In the cold tier branch, compute `const mode = description_mode ?? 'summary'` and apply `summarize` to each entry when `mode === 'summary'`.
   - In the warm/hot tier branches, default `mode` to `'full'` (or omit `description_mode` from the response for those tiers; either is acceptable).
   - Include `description_mode` in the response for cold tier only (or always; document choice).
...
- [ ] `loop_describe({ tier: 'cold' })` (no flag) returns descriptions truncated to 200 chars
- [ ] `loop_describe({ tier: 'cold', description_mode: 'full' })` returns full descriptions
- [ ] Cold-tier token count in summary mode ≤ 16K (down from 27K)
...
- **Risk**: warm tier defaults to `'full'` but some callers expect `'summary'` (e.g., a tool that already loaded the full data and doesn't need it again). → **Mitigation**: the warm tier is small (27 entries, ~5K tokens); full mode is fine. The flag is opt-in; callers can request `summary` if they want.
...
- **Risk**: cold tier summary mode breaks a tool that expected the full `description` field. → **Mitigation**: the `description_mode: 'full'` opt-in is the workaround. Existing tools that depend on cold tier full descriptions need to add the flag. (A scan of `__tests__/` for callers found 0 such tools at planning time.)
