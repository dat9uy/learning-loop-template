...
- **Functional:**
  ...
  - `metaStateProposeDesignTool` and `metaStatePromoteRuleTool`: verify (no code change). Both already use top-level fields.
  - Update the tool description strings for `meta_state_report` and `meta_state_log_change` to mention "top-level" fields.
...
3. **Verify `metaStateProposeDesignTool`.** Read the file. Confirm it does NOT have any `evidence.code_ref` or `evidence.code_ref` references. (Expected: yes, no change needed.)
4. **Verify `metaStatePromoteRuleTool`.** Read the file. Confirm the same. (Expected: yes, no change needed.)
...
- **Risk:** The tool description strings reference the old shape and confuse new agents. **Mitigation:** update both description strings in this phase.
