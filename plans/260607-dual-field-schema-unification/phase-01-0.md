...
1. **Decide record shape.** Use the same shape as `records/meta/decisions/decision-meta-260512T0046Z-loop-meta-evidence-gap-revisit.yaml` (an existing meta decision). Fields: `surface: meta`, `question`, `decision`, `rationale`, `alternatives` (mention the rejected options from the brainstorm), `tradeoffs`, `source_refs` (cite `local:meta-state:<id>` for the finding), `decision_effect`, `notes`.
