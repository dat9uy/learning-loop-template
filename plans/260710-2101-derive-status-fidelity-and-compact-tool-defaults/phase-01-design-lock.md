...
Read `core/query-drift.js` lines ~80-135 (the `derived_status`-driven drift/recommendation branches) and `tools/handlers/meta-state-derive-status-tool.test.js` (the acceptance test expectations). Confirm: (a) under Option B (`test_passed === true` required for `mechanism-shipped`), does `query-drift.js:87,90` (which treats both `resolved-by-mechanism` AND `active-uncertain` as drift) still fire drift detection for a "code exists, no test-pass signal" finding? Expected: yes — it derives `active-uncertain`, still drift, recommendation narrows to `investigate`. (b) Does the derive-status tool's acceptance test assert a `resolve` recommendation under `run_tests:false`? If yes, that test is intentionally flipped (matches the PR #47 pattern of flipping RED tests that documented broken behavior). **Locked: Option B** (Validation Session 1) — confirm in-code. Tests to flip: `sp1-derive-status-acceptance.test.js:42-51` (4 assertions incl. `drift:true`→`false`) + any `derive-status.test.js` cases asserting `mechanism-shipped`/`resolve` under default `run_tests:false`.
...
Read `runtime-state-record-tool.js` schema (lines ~28-37) to confirm the full row field set: `kind, affected_system, id, value, delta, source_ref, fingerprint, timestamp, status, metadata`. Grep for any consumer that reads `runtime_state_read` output `.metadata` or `.fingerprint` from the *default* (un-filtered, non-compact) path — search tests and `runtime_state_read` callers. **Lock:** the compact field whitelist (drop `metadata` + `fingerprint`) and confirm no default-path consumer breaks. Expect: no consumer relies on `metadata` from defaults; callers needing it pass `compact:false`.
