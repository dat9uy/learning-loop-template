...
blocks: []
related:
  - meta-state.jsonl entry meta-260606T1543Z-meta-state-cross-reference-field-design (Phase 5 closes this; becomes redundant under the new design)
  - meta-state.jsonl entry meta-260606T1531Z-cold-session-test-rule-deferred (one of the 3 design notes re-emitted in Phase 2)
  - meta-state.jsonl entry meta-260606T0421Z-instruction-layer-for-agents-tbd (one of the 3 design notes re-emitted in Phase 2)
  ...
  - meta-state.jsonl (registry; Phase 0 adds 1 change-log; Phase 2 mutates 4 promoted_to_rule findings + adds 4 rule entries + supersedes 3 design-note-in-disguise findings + adds 3 loop-design entries; Phase 5 resolves 1 cross-reference entry)
...
|-------|------|--------|--------|
| 0 | Surface the design (change-log + supersede 3 design-note findings) | Pending | 0.5h |
...
| 2 | Migration: 4 rules → `entry_kind: "rule"`; 3 design notes → `entry_kind: "loop-design"`; 3 findings superseded with `consolidated_into` (idempotent script) | Pending | 1.5h |
...
- [ ] Phase 0: 3 design-note-in-disguise findings are marked `status: "superseded"` with `consolidated_into: <reserved id from Phase 2>` (a placeholder; Phase 2 backfills the real loop-design id)
...
- [ ] Phase 2: migration script (`tools/learning-loop-mcp/scripts/migrate-rule-entry-kind.mjs`) extracts the 4 promoted rules into `entry_kind: "rule"` entries, mutates the 4 source findings' `promoted_to_rule` to the new rule id, and re-emits the 3 design notes as `entry_kind: "loop-design"` entries with `proposed_design_for` and `addresses` fields populated
- [ ] Phase 2: the 3 design-note-in-disguise findings' `consolidated_into` is backfilled to the real new loop-design entry id (replacing the Phase 0 placeholder)
...
- [ ] Phase 4: `loop_describe({ tier: "warm" })` returns `rule_count`, `loop_design_count`, and the existing `promoted_rules` field is renamed to `rules` and reads from the new entry kind
...
- The clean-break migration is explicit in Locked #7 (with the 3-superseded-findings-stay deviation)
...
- `tools/learning-loop-mcp/tools/meta-state-promote-rule-tool.js` — Phase 1 rewrites to emit `entry_kind: "rule"` entries
- `tools/learning-loop-mcp/tools/meta-state-list-tool.js` — Phase 3 accepts the new entry_kinds in its filter
- `meta-state.jsonl` — 40 entries at session start (Phase 0 adds 1 change-log; Phase 2 mutates 4 + adds 4 rule entries + supersedes 3 + adds 3 loop-design entries; Phase 5 resolves 1)
