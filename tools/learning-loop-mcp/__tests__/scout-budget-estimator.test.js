...
test("low risk: prompt with 0 file reads, 2 MCP calls", () => {
  const prompt = "Call mcp__learning_loop_mcp__loop_describe with tier=summary. Then call mcp__learning_loop_mcp__loop_describe with tier=warm.";
...
Read file b.md
Then mcp__learning_loop_mcp__a`;
  const est = estimateBudget("test.js", prompt, 60);
...
test("cold-session test 1 prompt reproduces the 1522Z hang estimate", () => {
  ...
  const prompt = `Read tools/learning-loop-mcp/server.js
Read tools/learning-loop-mcp/tools/manifest.json
Read tools/learning-loop-mcp/core/gate-logic.js
Read tools/learning-loop-mcp/core/meta-state.js
Read tools/learning-loop-mcp/agent-manifest.json
Read tools/learning-loop-mcp/core/patterns.json
