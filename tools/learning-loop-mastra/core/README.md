...
3. **Baseline storage.** `fallow` auto-creates `<root>/.fallow/.gitignore: *` that silently gitignores `.fallow/baselines/`. Verify `git ls-files <root>/.fallow/baselines/` returns expected files BEFORE committing. Prefer `<root>/baselines/fallow/` (committed, outside the auto-ignored cache). The previous `plans/<plan-slug>/reports/fallow/` location is **deprecated** because it couples CI to a plans directory (plans dirs are scratch artifacts; production CI must not depend on them). Baselines are package-scoped: each fallow-audited package gets its own `<root>/baselines/fallow/`.
4. **Third-party Action SHA pin.** When swapping a hand-rolled `pnpm exec <tool>` (or `npx <tool>`) for a third-party GitHub Action (`uses: <vendor>/<tool>@<ref>`), pin to commit SHA, NOT tag (`uses: <vendor>/<tool>@<sha>`). Verify the Action provides cryptographic signature verification (e.g., fallow-rs/fallow v2 uses Ed25519 + SHA-256 + sentinel at `npm/fallow/scripts/{verify-binary,lazy-verify,run-binary}.js`) and assert `verified: yes (<sentinel-path>)` in CI logs. Tag-based pins drift silently when upstream re-tags; SHA pins survive tag deletion. The CLI binary version is pinned separately via the `version:` input (or read from `package.json devDependencies.<tool>` via a setup step). Canonical example: `fallow-rs/fallow@<commit-sha>`.

Origin findings: `meta-260628T1328Z-commit-6f9402e-...` (item 1), `meta-260628T1328Z-fallow-dead-code-save-regression-baseline-...` (item 2), `meta-260628T1329Z-when-fallow-runs-...` (item 3). All three are FIXED in commit `9ed520d`; this section exists to prevent recurrence. Item 4 was added per plan `260629-2011-fallow-tools-v2-action-swap` (Phase 3) — the SHA-pin requirement was implicit in plan 260628-1337 but not codified until the fallow-tools Action swap exposed the gap.
