...
const PROCESS_HINTS = Object.freeze([
  "pnpm test discipline. `pnpm test` runs the full vitest suite; the json reporter writes `.test-logs/vitest-results.json` with shape numTotalTests/numFailedTests/numTotalTestSuites + testResults[].assertionResults[]; status field is passed/failed. Run targeted, not the full suite: `pnpm exec vitest run --bail=1` (iterate, stops at first failure), `pnpm exec vitest run <path>` (one file), `pnpm exec vitest --changed` (post-edit, files in `git diff`). Parse once via `bash tools/scripts/vitest-failures.sh` (exit 0 green / 1 failed / 2 missing-or-invalid; jq fallback if the script is absent: `jq -r '[.testResults[].assertionResults[] | select(.status==\"failed\") | {fullName, failureMessages}]' .test-logs/vitest-results.json`). Do NOT grep raw vitest stdout, re-read passing tests, or hand-write `python -c`/`node -e` to parse the JSON. Rule 2 (same-file-read): if you read the same file >5 times in 60s with no Edit/Write/Bash, STOP — write a one-line journal to `plans/reports/` and ask the operator.",
