 ...
 *
 * The gate log is a deliberate single-surface audit trail (the canonical
 * `.claude` runtime), distinct from the cross-surface `.gate-decision.log`.
 * The surface name is derived from `SURFACES[0]` so the primary runtime is
 * never hard-coded; `runtime-agnostic.test.js` pins the SURFACES ordering.
 */
function resolveLogDir(root) {
  ...
  return join(root, SURFACES[0], "coordination");
}
