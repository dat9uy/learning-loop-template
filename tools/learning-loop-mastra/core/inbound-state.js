 ...
 *
 * Plan 260724-1119 Phase 2 (R5): kind+status filter is load-bearing.
 * `readRuntimeStateRows` is NOT a kind filter — the caller must apply the
 * explicit `kind === "budget-state" && status === "active"` guard before
 * the per-observation staleness check. Ledger-event rows are out of scope
 * by kind (concept boundary, not an exemption the gate grants); budget-state
 * rows with paused/stopped/initial status are out by lifecycle.
 */
export function checkObservationStaleness(observations, root) {
  ...
  // Lazy-read sidecar once for all non-meta observations. Plan 260724-1119
  // R5: filter to budget-state+active rows only — ledger-event rows are
  // out of scope; budget-state rows with non-active lifecycle are out of
  // scope. `readRuntimeStateRows` does not apply this filter.
  let sidecarCache = null;
  function getSidecar() {
    if (sidecarCache === null) {
      sidecarCache = readRuntimeStateRows(root).filter(
        (r) => r && r.kind === "budget-state" && r.status === "active",
      );
    }
    return sidecarCache;
  }
