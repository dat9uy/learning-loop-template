...
await test("readGateOverride first-valid-wins prefers .claude over .hermes", () => {
  const claudePath = join(root, ".claude", "coordination", ".gate-override");
  const hermesPath = join(root, ".hermes", "coordination", ".gate-override");
  ...
  writeFileSync(hermesPath, JSON.stringify({ rule_ids: ["rule-hermes"], ttl_seconds: 3600, operator_note: "h", created_at: new Date().toISOString() }));
  const override = readGateOverride(root);
  assert.deepStrictEqual(override.rule_ids, ["rule-claude"]);
});

await test("readGateOverride falls through to .hermes when .claude marker is expired (first-VALID-wins, not first-parsed)", () => {
  // F-1 regression test: an expired marker on .claude must NOT shadow a
  // valid marker on .hermes. The old behavior (pre-Step 4 refactor) iterated
  ...
  writeFileSync(
    hermesPath,
    JSON.stringify({ rule_ids: ["rule-hermes"], ttl_seconds: 3600, operator_note: "h", created_at: new Date().toISOString() }),
  );
  const override = readGateOverride(root);
  assert.ok(override, "expected to fall through to valid .hermes marker");
  assert.deepStrictEqual(override.rule_ids, ["rule-hermes"]);
  assert.strictEqual(override.operator_note, "h");
});
