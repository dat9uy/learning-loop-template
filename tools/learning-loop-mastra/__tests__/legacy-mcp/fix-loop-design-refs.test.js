...
// INC-4 fix (plan 260704-0301-stale-findings-dispatch-handle): the previous
// version mutated the LIVE meta-state.jsonl on every pnpm test run (the
// pre-commit hook) by running fix-loop-design-refs.mjs against resolveRoot()
// with no GATE_ROOT override. Switching to mkdtempSync isolation: copy the
...
function setupTempRegistry() {
  tempDir = mkdtempSync(join(tmpdir(), "fix-loop-design-refs-"));
  copyFileSync(join(liveRoot, "meta-state.jsonl"), join(tempDir, "meta-state.jsonl"));
...
test("Phase 1: fix-loop-design-refs fixes broken refs and is idempotent", () => {
  ...
  try {
    ...
    const firstRun = execSync(`node ${scriptPath.pathname}`, {
      cwd: tempDir,
      stdio: "pipe",
      env: { ...process.env, GATE_ROOT: tempDir },
    }).toString();
    ...
    const secondRun = execSync(`node ${scriptPath.pathname}`, {
      cwd: tempDir,
      stdio: "pipe",
      env: { ...process.env, GATE_ROOT: tempDir },
    }).toString();
...
test("Phase 1: fix-loop-design-refs emits change-log entry", () => {
  setupTempRegistry();
  try {
    // Run the script once to populate the change-log entry in tempDir.
    execSync(`node ${scriptPath.pathname}`, {
      cwd: tempDir,
      stdio: "pipe",
      env: { ...process.env, GATE_ROOT: tempDir },
    });
