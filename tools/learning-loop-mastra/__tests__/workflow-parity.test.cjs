...
describe("workflow parity harness", () => {
  ...
  test("tools/list enumerates 33 mastra_* + 10 run_workflow_* = 43 mastra-and-workflow total", { timeout: 10000 }, async () => {
    ...
    assert.equal(mastra.length, 33, `must have 33 mastra_* tools (added meta_state_dispatch_finding), got ${mastra.length}`);
    assert.equal(runWorkflows.length, 10, `must have 10 run_workflow_* tools (8 existing + 2 storage), got ${runWorkflows.length}`);
    assert.equal(tools.length, 46, `total must be 46 (33 mastra_* + 10 run_workflow_* + 3 ask_*), got ${tools.length}`);
