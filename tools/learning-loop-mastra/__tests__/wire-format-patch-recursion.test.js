...
test("mastra_meta_state_patch accepts flat patch object via stdio", async () => {
...
// Test 2: coerceParams unit test — double-nested unwrap.
test("coerceParams unwraps {item: {item: [...]}} chain", () => {
  const schema = {
    shape: {
...
// Test 3: meta_state_propose_design via coerceParams with real schema.
...
test("coerceParams unwraps {item: [...]} in real propose_design schema", () => {
...
test("coerceParams unwraps {item: []} to flat empty array", () => {
  const realSchema = metaStateProposeDesignTool.schema;
  const result = coerceParams(
    {
