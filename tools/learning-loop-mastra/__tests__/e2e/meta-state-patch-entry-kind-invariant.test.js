...
// All tests run through the CLI (bin/loop.mjs + adaptLegacyHandler) so the
// Zod validation + .default() injection fires exactly as it did over MCP. The
// single-surface contract keeps these tools on the CLI; MCP no longer
// registers them. Registry state remains the load-bearing invariant.
//
...
// or identity key no longer throws at the boundary. Identity fields
// (entry_kind, status, ...) are denied at the handler as a normal result
// {patched:false, reason:"immutable_field"|"empty_patch", ...} — the CLI
// returns it with exit 0. Tests assert the result fields AND the registry
// state (entry_kind must not flip) as the load-bearing invariant.

function makeRoot() {
  const root = mkdtempSync(join(tmpdir(), "patch-kind-cli-"));
...
test("meta_state_patch empty patch {} on loop-design is rejected with reason empty_patch; entry_kind unchanged", () => {
  const tempRoot = makeRoot();
  const design = runCli("meta_state_propose_design", {
  ...
  const result = runCli("meta_state_patch", {
    id: design.id,
...
//     at-invocation error payloads: the CLI-layer `patch` field is now a free-form
//     z.record (H10), so an unknown/identity key no longer throws at the boundary.
//     The identity invariant is enforced deeper — the handler strips entry_kind
//     and returns {patched:false, reason:"immutable_field", denied_fields:["entry_kind"]}
//     instead of throwing. The registry-state assertion (entry_kind must not flip)
//     remains the load-bearing check.
test("meta_state_patch rejects entry_kind inside patch (Fix A); registry state unchanged", () => {
  const tempRoot = makeRoot();
  ...
  // matches the rule branch, flips to "rule". See red-team Failure #3.)
  const result = runCli("meta_state_patch", {
    id: report.id,
    entry_kind: "finding",
...
test("meta_state_patch empty patch logs reason=empty_patch, no fields_patched", () => {
  const tempRoot = makeRoot();
  const report = runCli("meta_state_report", {
    category: "loop-anti-pattern",
    severity: "warning",
