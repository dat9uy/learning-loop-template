...
function makeRoot() {
  const root = mkdtempSync(join(tmpdir(), "patch-derived-cli-"));
  mkdirSync(join(root, "records", "meta", "decisions"), { recursive: true });
  // Copy schemas so core modules resolve.
...
test("meta_state_patch ACCEPTS wrapped {item: [...]} on proposed_design_for and stores flat array", () => {
  const tempRoot = makeRoot();
  // 1. Create a loop-design entry.
  const designResult = runCli("meta_state_propose_design", {
  ...
  const patchResult = runCli("meta_state_patch", {
    id: designId,
...
test("meta_state_patch ACCEPTS wrapped {item: [...]} on addresses and stores flat array", () => {
  const tempRoot = makeRoot();
  // 1. Create a loop-design entry.
  const designResult = runCli("meta_state_propose_design", {
...
test("meta_state_patch flat proposed_design_for round-trips as flat array", () => {
  const tempRoot = makeRoot();
  // 1. Create a loop-design entry.
  const designResult = runCli("meta_state_propose_design", {
