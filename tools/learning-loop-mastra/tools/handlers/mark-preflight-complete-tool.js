...
export const gateMarkPreflightTool = {
  ...
  schema: {
    surface: z.enum(["product", "skills", "schemas", "runtime-tracking", "runtime-state"]).describe("Surface to mark as preflight-complete. Currently 'product' (unlocks product/** writes), 'skills' (unlocks <runtime-surface>/skills/** writes), 'schemas' (unlocks schemas/** writes), 'runtime-tracking' (unlocks runtime_state_pause/resume/stop), and 'runtime-state' (unlocks runtime_state_record). The marker is named .loop-preflight-<surface>; the write gate reads it back for matching paths."),
  },
