...
export const gateMarkPreflightTool = {
  ...
  description: "Mark the preflight checklist as completed for a given surface. This is the ONLY way to create a preflight marker — direct file writes to .loop-preflight-* are blocked by the write gate. After calling this tool, writes to that surface's gated paths are unlocked for 30 minutes. The marker file (.loop-preflight-<surface>) is stored in each runtime's coordination/ dir (.claude, .factory, .mastracode) and has a 30-minute TTL. Use when you are about to write to gated paths — `product/**` (surface: \"product\"), `<runtime-surface>/skills/**` (surface: \"skills\"), `schemas/**` (surface: \"schemas\"), or `runtime-state.jsonl` (surface: \"runtime-state\", including shell-side row maintenance) — and have walked through the 6-step preflight checklist. Not for record CRUD (records are gated differently) and not for general command checks (use `gate_check` instead).",
  schema: {
    surface: z.enum(["product", "skills", "schemas", "runtime-tracking", "runtime-state"]).describe("Surface to mark as preflight-complete. Currently 'product' (unlocks product/** writes), 'skills' (unlocks <runtime-surface>/skills/** writes), 'schemas' (unlocks schemas/** writes), 'runtime-tracking' (unlocks runtime_state_pause/resume/stop), and 'runtime-state' (unlocks runtime_state_record AND direct writes to runtime-state.jsonl on both the write-gate and bash-gate paths). The marker is named .loop-preflight-<surface>; the write gate and bash gate read it back for matching paths."),
  },
