...
const LOCAL_DISCOVERABILITY_HINTS = Object.freeze([
  ...
  "Findings have 6 statuses: `reported` (24h TTL), `active` (operator-acked), `resolved` (closed), `expired` (legacy — kept for backward compat; new TTL semantics use `stale`), `superseded` (consolidated into a change-log).",
  "For reopens: set reopens: ['<old_expired_id>'] on the new finding at report time, then cascade-resolve the parent via meta_state_resolve({id: old_id, cascade_from: [child_id]}).",
