...
All gate logic lives in `tools/learning-loop-mastra/core/` (ESM, single source of truth). Hook wrappers delegate to universal ESM hooks via `execFileSync`. There is no shared CJS utility file — the thin wrappers intentionally contain zero business logic.
