...
// Test 4: docker run → exit 2 (constrained, no observation with constraint_type: docker)
{
  ...
  assert(output && output.decision === 'block', 'docker run output has decision: block');
}
