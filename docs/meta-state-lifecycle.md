...
| `citation` | Asserted-relationship carrier for the collapsed edges (`origin`, `supersedes`, `consolidated_into`) | Always `active`; immutable audit row | Permanent: never archived |
...
**Terminal** (single set): `{accepted, resolved, archived}`. The Zod enum on `status` includes `archived`; the schema-enum terminal set and the predicate-effective terminal set (`core/constants.js`, consumed by `isOpen`) are identical.

**Non-terminal**: `open`. It has **staleness pressure** as a derived view (`isStaleView`), not a status: a stale-view `open` finding is re-verifiable via `meta_state_re_verify`. There is no `auto-resolved` status (removed).

Note: `core/meta-state.js` has a local `TERMINAL_STATUSES = {resolved, accepted}` (without `archived`) used internally as the open-predicate helper for the write path. It is NOT the schema-enum terminal set — do not conflate the two.
...
- Inactive and archived Rules remain in the registry for lineage. Material refinements append a new version under the stable id and emit a `supersedes` citation to the prior Rule; the on-record `supersedes` field is inert-historical.
...
- **Vocabulary axis (`pattern_type` ↔ consumption state).** `pattern_type` names the consumption axis: `agent-checklist` Rules are state-2 — deterministic injection via a `PROCESS_HINTS` row in `core/loop-introspect.js`, enforced by the H6 ordering gate in `loop_describe`, plus agentic consumption (the model interprets the checklist). `determinism-checklist` Rules are state-3 — the `meta_state_resolve` consult-gate evaluates them deterministically and blocks on drift (`rule-no-orphaned-evidence`). **`regex` and `glob` survive only for I3 action-boundary Rules** — match-language, state-3: `regex` matches bash commands, `glob` matches write paths. The advisory Rules previously typed `agent + regex/glob` were `agent`-skip-by-`applyPromotedRules` regardless of their pattern body (no command/path matching), so reclassifying them to `agent-checklist` with checklist bodies eliminates dead match specs without changing gate behavior. `internalization_level` names the consumption contract: `I3` = state-3 deterministic action-boundary consumption, `I2` = state-2 deterministic delivery with agentic judgment. The concept term `consult-gate` (in `docs/loop-engine.md` and `docs/philosophy.md`) is preserved on the L1 concept surface; it is now lexically distinct from `agent-checklist`.
