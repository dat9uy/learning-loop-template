...
All 10 test namespaces pass: 1069 pass / 0 fail / 1 skip. Two findings resolved, one change-log filed for the master tracker flip. Plan 1a is closed; Plan 1b (CR-3 to CR-6 hygiene) and Plan 3 (C6+C7 cut-over) are next.
...
The `consolidated_into` inverse index omission is equally embarrassing. `buildInverseIndexes` in `loop-introspect.js` had 5 maps: `addresses_inverse`, `origin_inverse`, `promoted_to_rule_inverse`, `reopens_inverse`, and `supersedes_inverse`. The `consolidated_into` relationship — which is the forward `change-log.consolidates` field — was simply never added. So querying a change-log's inbound relationships showed `addressed_by`, `origin_of`, `promoted_from`, `reopened_by`, and `superseded_by`, but not `consolidated_by`. This broke the traceability chain from a finding to the change-log that consolidated it. A 6th map is trivial; the oversight is not.

The zod pin and mutex fixes are code-review debt from PR #3 (Plan 2). The caret pin was flagged by the code reviewer as CR-1; the mutex bypass was CR-2. We shipped Plan 2 with these known gaps because the 10-namespace test suite was green and the findings were the immediate priority. That was the right call, but it means Plan 1a was always going to be a cleanup PR. The stacked-commit pattern (Phase B's B3+B4 → B5 → B6) worked again, but it is exhausting to keep re-proving that atomic fixes are the right shape.

## Technical Details

- **Finding 1 fix location**: `tools/learning-loop-mcp/tools/meta-state-list-tool.js:12-14` — `EXCLUDABLE_STATUSES` set defined; `179-186` — filter logic now skips entries whose status is in `EXCLUDABLE_STATUSES` unless the caller filters by one of those statuses or sets `include_archived: true`.
