...
**Reclassification (portable six).** The 6 single-step deterministic workflow tools (`workflow_classify_prompt`, `workflow_prepare_runtime_request`, `workflow_self_improvement`, `workflow_intentional_skip`, `workflow_report_phase_status`, `workflow_runtime_probe`) are **CLI-capable** and now ride the CLI under `LOOP_RECORDS_VIA_CLI=1`: each was unwrapped from `createLoopWorkflow` into a plain `tools/manifest.json` handler with the shared `wrapWorkflowInputSchema` envelope contract, and joins `CLI_WRITE_TOOLS` as a stateless pure transform (not a record-surface write). Only `workflow_generate_prompt` remains `deferred-rehoming` (cross-root blueprint resolution). **Multi-step note (for future re-homings):** a multi-step deterministic workflow re-homed to the agent home requires gate-observed (not agent-asserted) step-success to keep ordering enforceable. The portable six are single-step, so this does not apply now; the contract is stated for any future re-homing.
