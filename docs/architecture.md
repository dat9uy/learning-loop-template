...
The tool surface is 43 manifest entries (`tools/learning-loop-mastra/tools/manifest.json`). 41 of them ride the CLI as `CLI_TOOLS` (12 read `CLI_READ_TOOLS` + 29 write `CLI_WRITE_TOOLS` — see `core/cli-tools.js`, or run `LOOP_SURFACE=.claude node tools/learning-loop-mastra/bin/loop.mjs list`). All three wired runtimes (`.claude` via root `.mcp.json`, `.factory`, `.mastracode`) set `LOOP_RECORDS_VIA_CLI=1`, so both reads and writes ride the CLI and the MCP server keeps only a 2-tool residue: `workflow_generate_prompt` and `check_runtime_agnostic`, plus operator-only storage/allowlist/audit handlers (`update_r2_allowlist` and friends) that are not in the tool manifest. All policy logic lives in `tools/learning-loop-mastra/core/` — single source of truth for all runtimes.
...
- **Production injection:** `core/loop-introspect.js` builders (`buildDiscoverabilityHints` / `buildProcessHints`) project the registry into the legacy array-of-strings shape. All injection surfaces consume the builders — the hint renderer is NOT on the injection path (the builders already deliver single-source content; wiring hooks through the renderer would churn three hot paths for no behavioral gain).
