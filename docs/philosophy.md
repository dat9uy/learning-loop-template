...
**The encode-or-prose rule.** The two-axis model tells you the *states*; it does not by itself tell you *when to move something down the gradient*. That decision is one sentence: **a distinction among values earns its keep only when deterministic code branches on it (state-3 consumption).** If no code branches on a value — no gate fires differently, no status derives differently, no tool routes by it — the distinction is consumed *agenticly* (the model reads the records and interprets), so it belongs in **prose**, not a validated field or an encoded mechanism. This is the refactor lens: when you consider encoding a rule, promoting a finding, or adding a strict schema field, name the deterministic branch that consumes it. If you cannot, it is state-1/2 prose and should stay there. The same rule, restated for the doc layers (L1/L2/L3): encode a claim as L2/L3 mechanism only when code branches on it; otherwise it stays L1 concept prose (see `docs/loop-engine.md` § Layer model). The deep treatment is below (§ Schema Constraints Are State-3 Artifacts).
