...
The `kind` discriminator (`ledger-event` | `budget-state`) at the mechanism surface anticipates this split. The honest state of the implementation today: the discriminator is not yet load-bearing — the gate stale-scans every active row regardless of kind, and budget tracking is recorded as `ledger-event`. Making `kind` load-bearing (gate scopes to budget tracking; ledger logs are audit-only) and giving budget tracking an in-band versioned lifecycle (so `pause`/`stop` are lifecycle records, not a sidecar toggle, and `stop` retires tracking while preserving history — replacing the destructive `prune` for "I am done tracking this budget") is the open direction; see `docs/architecture.md` § Per-surface tracking toggle + prune for the current mechanism.
