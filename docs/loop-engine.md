...
**Status of the cycle (state this inline, not in a footnote):** the cycle is half-wired. The record step and the promotion step exist today. Recurrence detection over **gate decisions** is wired — a gate event files a recurrence finding only when the evaluator proves an `unexpected-match` (parser-proven inert data); ordinary rule fires are deterministic gate telemetry, not findings. The unwired half is recurrence detection over **agentic-deferral change-logs**: which deferrals recur and are ready to promote is not yet detected mechanically, so promotion stays operator-triggered. A human decides when to promote. Closing the change-log half is the open design question at the end of this doc, not a solved part of the invariant.
...
## Budget tracking vs ledger log (runtime-state)

`runtime-state.jsonl` carries two row kinds that share a store but must not share semantics. Naming the distinction is load-bearing: conflating them is the root cause of the "delete the ledger to clear the gate" class of flaw.

- **Budget tracking** — mutable state for an external resource budget (a device-slot allowance, an API quota). It has a **tracking lifecycle** the operator owns: `initial` (start tracking the budget) → `active` → `paused` (suspend tracking — the operator's statement that this budget rule no longer matters *for now*) → `stopped` (retire tracking). The inbound gate's stale-observation scan is *for this kind*: a stale budget row means "the recorded external-resource state may have drifted; reconcile before acting." `pause`/`stop` cease the side effect **by design** — a budget not being tracked is not scanned — not by suppression. This is the natural lifecycle of external-budget management, not a filter applied to noise.
- **Ledger log** — immutable audit of an event that happened (a steering delivery recorded, a finding dispatched, an experiment run). It has **no tracking lifecycle**: it is history, not state. Stale-scan is a category error on a ledger log — you do not reconcile a delivery record against external state, and you do not delete history to quiet a budget gate. Ledger logs are out of the budget gate's scope by definition (concept boundary), not by an exemption the gate grants.

**Durability is a separate axis from kind, and the operator must know it** — it decides whether a record outlives the session that wrote it. Ledger logs are durable history by definition. Budget tracking splits along this axis: the *tracking lifecycle* (`initial`/`active`/`paused`/`stopped` under the canonical id) is the operator's durable, shared statement about an external budget — it outlives any session and must read identically to every participant; a *TTL'd allowance* (a session-scoped authorization such as a gate-verb allowance) belongs to the session that minted it and expires — it is not durable and does not outlive its session. Conflating the two — committing an ephemeral allowance as if it were durable history — is the same class of flaw as deleting a ledger to clear a gate: both misread the durability axis.

The `kind` discriminator (`ledger-event` | `budget-state`) at the mechanism surface is load-bearing: the gate's stale scan and drift check scope to `budget-state` rows with `status: active`; budget tracking carries an in-band versioned lifecycle on `status` (`pause`/`stop` are lifecycle records, not a sidecar toggle); and `stop` retires tracking while preserving history — the non-destructive replacement for the removed `prune`. Rows written before the discriminator existed (no `kind` field) are read as budget-state so legacy sidecars never go silently dark. See `docs/architecture.md` § Per-surface tracking toggle and `docs/runtime-contract.md` for the mechanism.
